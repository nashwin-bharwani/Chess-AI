...
public class Board {
	...
	static double[] pieceMobility = new double[] { 0, 0.5, 0.75, 1, 1.5, 0.4 };
	...
	public double evaluate(boolean AIColor, boolean isEven) {
			...
			for (Piece nextPiece : newRow) {
				if (nextPiece != null) {
					...
					if (AIColor == nextPiece.color) {
						...
						pieceCounts[ID]++;
						// Counts the mobility
						mobilityCounts[ID] += nextPiece.generateValidMoves().size();
						// Counts the threats
		...
		if (isEven) {
			for (int loopCount = 0; loopCount < 6; loopCount++) {
				...
				score += mobilityCounts[loopCount] * pieceMobility[loopCount];
	...
	public void moveTo(Piece selectedPiece, int toX, int toY) {
		...
		if (selectedPiece.identity == PAWN) {
			...
			if (toY == 7 || toY == 0) {
				...
				System.out.println("Choice of Promotion: " + choice);
