...
public class Pawn extends Piece {
	...
	public Set<Point> generateValidMoves() {
		...
		if (canEnPassantLeft && safeEnPassant(row - 1, col)) {
			validMoves.add(new Point(row - 1, col + colShift));
			// System.out.println(row+" "+col);
		}

		// Determine en passant right.
		if (canEnPassantRight && safeEnPassant(row + 1, col)) {
			validMoves.add(new Point(row + 1, col + colShift));
			// System.out.println(row+" "+col);
		}
	 ...
	 * determine if it will be allowed. this method assumes the en passant is
	 * possible without regard to the king, and ensures that it will not be in
	 * check afterwards, constituting a safe en passant.
	 * 
	 * @param enemyPawnRow
