...
public class AIMain {
	...
	static int noOfDeepness = 4;
	private Board board;
	public Move bestMove;
	public boolean AIColor;

	/**
	 * Create a new AIMain object which stores the board and the color of AI
	 * 
	 * @param b
	 *            the current Board object
	 */
	public AIMain(Board b, boolean color) {
		this.board = new Board(b, false);
		AIColor = color;
	}
	
	public static void updateDeepness(int newDeepness){
		noOfDeepness = newDeepness;
	}
	...
	public double alphaBetaMinimax(Board b, int currentPly, boolean isAITurn,
		...
		if (currentPly % 2 == 1) {
			...
			for (Move m : possibleMoves) {
				...
				double result = alphaBetaMinimax(b.makeMove(m),
						currentPly, false, alpha, beta);
				b.undoMove();
